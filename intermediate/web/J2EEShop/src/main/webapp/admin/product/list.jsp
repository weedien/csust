							...
							<table cellspacing="0" cellpadding="1" rules="all"
								...
								<c:forEach var="item" varStatus="status" items="${pageBean.data}">
									<tr onmouseover="this.style.backgroundColor = 'white'"
										...
										<td style="CURSOR: hand; HEIGHT: 22px" align="center"
										    width="17%">
											<img width="40" height="45" src="${pageContext.request.contextPath}/${item.pimage}">
										</td>
										<td style="CURSOR: hand; HEIGHT: 22px" align="center"
										...
										<td align="center" style="HEIGHT: 22px">
											<a
												href="${pageContext.request.contextPath}/adminProductServlet?method=productDelete&pid=${item.pid}">
				...
				<td style="CURSOR: hand; HEIGHT: 22px" align="center" width="18%">
					<div id="paginationId" style="font-size:14px;margin:0;display:block;">
						...
						<c:if test="${pageBean.pageNumber gt 1}">
							...
							<a
								href="${pageContext.request.contextPath}/adminProductServlet?method=productList&pageNumber=${pageBean.pageNumber-1}">
								[上一页]</a>
						</c:if>&nbsp;
						...
						<c:forEach var="i" begin="1" end="${pageBean.totalPage}">
							...
							<c:if test="${pageBean.pageNumber != i}">
								<a
									href="${pageContext.request.contextPath}/adminProductServlet?method=productList&pageNumber=${i}">第${i}页</a>
							</c:if>
						</c:forEach>&nbsp;
						
						<c:if test="${pageBean.pageNumber lt pageBean.totalPage}">
							<a
								href="${pageContext.request.contextPath}/adminProductServlet?method=productList&pageNumber=${pageBean.pageNumber+1}">[下一页]
							</a>&nbsp;
							<a
								href="${pageContext.request.contextPath}/adminProductServlet?method=productList&pageNumber=${pageBean.totalPage}">
								[尾页]</a>
						</c:if>
					</div>
				</td>
